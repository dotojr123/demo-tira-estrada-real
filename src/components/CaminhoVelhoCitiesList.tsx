...
interface CaminhoVelhoCitiesListProps {
  title: string;
  subtitle: string;
  cities: string[];
}

const CaminhoVelhoCitiesList: React.FC<CaminhoVelhoCitiesListProps> = ({ title, subtitle, cities }) => {
  ...
  // Função para determinar o link correto da cidade
  const getCityLink = (cityName: string) => {
    // Cidades com páginas internas específicas
    if (cityName === 'Tiradentes') {
      return '/cidade/tiradentes';
    }
    if (cityName === 'São João del-Rei') {
      return '/cidade/sao-joao-del-rei';
    }
    if (cityName === 'Glaura') {
      return '/cidade/glaura';
    }
    
    // Outras cidades usam o padrão genérico
    return `/caminhos/velho/${generateSlug(cityName)}`;
  };
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {currentCities.map((city, index) => (
              <div
                ...
                data-city-name={city}
                onClick={() => window.location.href = getCityLink(city)}
              >
                {/* City Image */}
                <div style={{ 
                  width: '100%', 
...
export default CaminhoVelhoCitiesList;