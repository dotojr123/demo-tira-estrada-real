...
interface CaminhoCitiesListProps {
  title: string;
  subtitle: string;
  cities: string[];
}

const CaminhoCitiesList: React.FC<CaminhoCitiesListProps> = ({ title, subtitle, cities }) => {
  ...
  const getCityLink = (cityName: string) => {
    ...
    if (cityName === 'Conceição do Mato Dentro') {
      return '/cidade/conceicao-do-mato-dentro';
    }
    
    // Outras cidades usam o padrão genérico
    return `/caminhos/diamantes/${generateSlug(cityName)}`;
  };
...
export default CaminhoCitiesList;